...
namespace CodeFactory.IDE.VisualStudio.ProjectSystem
{
    ...
    /// </summary>
    public abstract class VsReference:VsProjectSystemModel,IVsReference
    {
        ...
        protected VsReference(bool isLoaded, bool hasErrors, IReadOnlyList<ModelException<ProjectSystemModelType>> modelErrors,
            string name,string filePath, ProjectReferenceType type, IReadOnlyList<string> aliases) 
            : base(isLoaded, hasErrors, modelErrors, ProjectSystemModelType.Reference, name)
        {
            _filePath = filePath;
            _type = type;
